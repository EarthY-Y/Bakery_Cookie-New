...
export const createOrders = async (req, res) => {
    try {
        ...
        //หาว่าเดือนนี้ผู้ใช้สั่งสินค้าไปกี่ครั้ง เพื่อเอาไป gen เป็น order_id
        const countOrder = await new Promise((resolve, reject) => {
            db.query(`SELECT COUNT(orders_id) AS order_count FROM orders 
                WHERE customer_id = ? 
                ...
                (err, result) => {
                    if (err) return reject(err);
                    resolve(result[0]?.order_count || 0); // ถ้าไม่มี order ให้ใช้ 0
                }
            );
        });
        
        // เรียกใช้ generateOrderId โดยส่ง countOrder ไปด้วย
        const orderId = await generateOrderId(phone_number, countOrder + 1);
        ...
        if (totalCostProduct) {
            ...
            if (results) {
                ...
                const resultsInsertCostProductDeatails = await new Promise((resolve, reject) => {
                    db.query("INSERT INTO cost_product_details (cost_product_details_id, order_cost_details_id, product_id, cost_product, quantity, total_product_cost) VALUES ?",
                        [costProductDetails],
                        (err, result) => {
                            if (err) return reject(err);
                            resolve(result);
