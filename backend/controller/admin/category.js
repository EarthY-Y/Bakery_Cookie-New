...
export const getListCategory = async (req, res) => {
    try {
        const results = await new Promise((resolve, reject)=> {
            db.query("SELECT c.product_category_id, c.category_name, c.created_at, c.updated_at, a_created.userName AS created_by,"+
...
export const craeteCategory = async (req, res) => {
    try {
        ...
        const results = await new Promise((resolve, reject)=> {
            db.query("INSERT INTO product_category (product_category_id, category_name, created_by) VALUES(?, ?, ?)", [idCategory, categoryName, token.admin_id],
                    (err, result) => { 
                if (err) return reject(err)
                resolve(result)
            })
    ...
    try {
        for (const item of selectedProducts) {
            ...
            await new Promise((resolve, reject) => {
                db.query(
                    "INSERT INTO category_product (category_product_id, product_category_id, product_id, created_by) VALUES (?, ?, ?, ?)", 
                    dataCategoryProduct, 
                    (err, result) => { 
                        if (err) return reject(err);
                        resolve(result);
    ...
    try {
        ...
        if(changesCategoryProduct.removed.lenght !==0){
            for (const item of changesCategoryProduct.removed) {
                await new Promise((resolve, reject) => {
                    db.query(
                        "DELETE FROM category_product WHERE product_category_id = ? and product_id =  ?", [idCategory, item.product_id], 
                        (err, result) => { 
                            if (err) return reject(err);
                            resolve(result);
                        }
        ...
        if(changesCategoryProduct.category_name){
            await new Promise((resolve, reject) => {
                db.query(
                    "UPDATE product_category SET category_name = ? WHERE product_category_id = ?", [changesCategoryProduct.category_name, idCategory], 
                    (err, result) => { 
                        if (err) return reject(err);
                        resolve(result);
                    }
        ...
        const results = await new Promise((resolve, reject) => {
            db.query(
                "UPDATE product_category SET updated_by = ? WHERE product_category_id = ?", [token.admin_id, idCategory], 
                (err, result) => { 
                    if (err) return reject(err);
                    resolve(result);
                }
