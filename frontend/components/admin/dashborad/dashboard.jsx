...
import { amoutOrdersService, salesService, newCustomerService, growthUpSalesService, salesPerMonthService } from '../../../API/admin/dashboradService';
...
const Dashboard = () => {
  ...
  const [salesData, setSalesData] = useState(0); // เริ่มต้นเป็น 0 เพื่อให้รองรับกับข้อมูลที่จะเอามาใส่
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const currentMonthData = getAllSales.data.find(sale => sale.sale_month === currentMonth);
        ...
        setSalesData(currentMonthData ? currentMonthData.total_sales : 0); 
        ...
        setSalesPerMonth(getSalesPerMonth.data)
      } catch (error) {
        console.error(error);
      } finally {
        setIsLoading(false);
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="row mb-5">
        ...
        <div className="col-12 col-sm-6 col-xl-3">
          <StatCard name="ยอดขายรายเดือน" icon="bi bi-bag" value={salesData > 0 ? numberGrouping(salesData): 0} color="success" />
        </div>
        <div className="col-12 col-sm-6 col-xl-3">
        ...
        <div className="col-12 col-lg-6 mb-4" >
          <div className="card shadow-sm border-0">
            <div className="card-body text-center">
              ...
              <div className="text-center py-5" style={{ display: "flex",justifyContent: "center",alignItems: "center"}}><SalesProductPieChart dataSales={salesPerMonth}/></div>
            </div>
          </div>
        </div>
      </div>
