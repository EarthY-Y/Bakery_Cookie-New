...
const salesProductPieChart = ({ dataSales }) => {
  ...
  // เตรียมข้อมูลเพื่อสร้างกราฟ
    // กรองข้อมูลที่มี total_quantity เป็น null ออก
    const filteredDataSales = dataSales.filter((item) => item.total_quantity !== null);

    // หากไม่มีข้อมูลให้แสดงข้อความ
    if (filteredDataSales.length === 0) {
      return <p className="text-center">ไม่มีข้อมูลสำหรับแสดงกราฟ</p>;
    }
  const labels = filteredDataSales.map((item) => item.product_name);
  const totalQuantity = filteredDataSales.map((item) => item.total_quantity);
