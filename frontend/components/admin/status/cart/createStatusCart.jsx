...
const createStatusCart = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const res = await CreateStatusCartService(statusName,"cart"); //จะส่งไปเป็น formData ได้ต้องผ่าน multer ก่อน
      navigate(-1);
      console.log(res);
    } catch (err) {
      console.log(err);
...
export default createStatusCart;