...
const editCartStatus = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(statusName, active);   
    try {
      const res = await updateStatusCartServiceById(id, statusName, active); //จะส่งไปเป็น formData เเบบนี้ได้ต้องผ่าน multer ก่อน
      navigate(-1);
      console.log(res);
    } catch (err) {
      console.log(err);
